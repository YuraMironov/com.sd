...
for ($i = 0; $i < $managers_count; $i++) {
	$newManager = new CCManager('manager' . ($i + 1));
	$work_days = rand(1, 7);
	for ($j = 0; $j < $work_days; $j++) {
		$work_day = new CCDay();
		$calls = rand(4, 10);
		for ($k = 0; $k < $calls; $k++) {
			$work_day->addScore(rand(2,5));
		}
...
if (count($center->managersIsGreatestThan(3)) == 0) {
	echo "please update";
} else {
	foreach ($center->managersIsGreatestThan(3) as $manager) {
